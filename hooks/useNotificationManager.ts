...
import { Organization, AppNotification } from '../types';
import { getSwedishHolidays } from '../data/holidays';
import { getSystemAnnouncements } from '../services/firebaseService';

export const useNotificationManager = (organization: Organization | null) => {
    ...
    useEffect(() => {
        ...
        const runChecks = async () => {
            ...
            const lastCheck = lastCheckStr ? new Date(lastCheckStr) : new Date(now.getTime() - 24 * 60 * 60 * 1000);
            ...
            allScreens.forEach(screen => {
                ...
                (screen.posts || []).forEach(post => {
                    ...
                    if (post.startDate && !post.endDate) {
                        ...
                        if (new Date(post.startDate) < fourteenDaysAgo) {
                            currentLocalNotifications.push({
            ...
            const lastCheckUnusedEvents = lastCheckUnusedEventsStr ? new Date(lastCheckUnusedEventsStr) : thirtyDaysAgo;

            if ((!organization.customEvents || organization.customEvents.length === 0) && lastCheckUnusedEvents <= thirtyDaysAgo) {
                const year = now.getFullYear();
            ...
            const reconciledLocalNotifications = currentLocalNotifications.map(newNotif => {
                const oldNotif = previousNotifications.find(n => n.id === newNotif.id);
