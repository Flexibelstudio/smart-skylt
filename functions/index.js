...
exports.runAiAutomations = onSchedule({
    ...
    console.log(`Running AI Automations check at ${now.toISOString()}`);
    ...
    const promises = orgsSnapshot.docs.map(async (doc) => {
        ...
        for (const automation of updatedAutomations) {
            ...
            if (lastRun) {
                ...
                let minInterval = 0;
                if (automation.frequency === "daily") minInterval = 23 * 3600 * 1000;
                if (automation.frequency === "weekly") minInterval = 6.5 * 24 * 3600 * 1000;
                if (automation.frequency === "monthly") minInterval = 27 * 24 * 3600 * 1000;

                if (timeSinceLastRun > minInterval) {
                    shouldRun = true;
                    // Additional checks for specific days could be added here for more precision
                }
            }

            if (shouldRun) {
                console.log(`Running automation "${automation.name}" for org "${org.name}"`);
                try {
                    // 1. Gather feedback history
                    const history = (org.suggestedPosts || [])
                        .filter((p) => p.automationId === automation.id)
                        .slice(-10); // Limit to last 10 suggestions
...
${feedbackContext}`;
                        try {
                            ...
                            console.log(`Generated insight for automation "${automation.name}": ${automation.latestInsight}`);
                        } catch (insightErr) {
                            console.error(`Could not generate insight for automation "${automation.name}":`, insightErr);
                            // Don't fail the whole run, just skip the insight
                        }
                    }
...
9. 'textAnimation': 'none', 'typewriter', 'fade-up-word', 'blur-in'.`;
                    ...
                    console.error(`Error running automation ${automation.id} for org ${org.id}:`, err);
                }
            }
        }

        if (hasChanges) {
            console.log(`Updating org ${org.id} with ${newSuggestions.length} new suggestions.`);
            const updatePayload = {
    ...
    console.log("AI Automations check finished.");
});
