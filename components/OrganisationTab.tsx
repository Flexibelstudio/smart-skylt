...
import { ChevronDownIcon, PencilIcon, TrashIcon, FacebookIcon } from './icons';
...
export const OrganisationTab: React.FC<SuperAdminScreenProps> = (props) => {
    ...
    const handleFacebookConnect = () => {
        ...
        if (!clientId || clientId.startsWith('YOUR_')) {
            alert("Facebook App ID är inte konfigurerat. Uppdatera public/config.js med ett giltigt App ID från Meta for Developers.");
            return;
        }
        ...
        <div className="space-y-8">
             ...
             <Card title="Sociala Medier" subTitle="Koppla innehåll från sociala medier.">
                ...
                <div className="pt-4 mt-4 border-t border-slate-200 dark:border-slate-700">
                    <h4 className="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1">Steg 1: Anslut till Instagram</h4>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">
                        För att kunna hämta händelser (Stories) måste du först ge Smart Skylt behörighet. Klicka på knappen och logga in med det Facebook-konto som är administratör for ditt Instagram Business-konto.
                    </p>
                    <PrimaryButton
                ...
                <div className="pt-4 mt-4 border-t border-slate-200 dark:border-slate-700">
                    <label htmlFor="insta-stories-id" className="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1">Steg 2: Instagram Business Account ID</label>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">När du har anslutit ditt konto, klistra in ditt ID här. <a href="https://www.facebook.com/business/help/1503421739714571" target="_blank" rel="noopener noreferrer" className="text-primary underline">Hur hittar jag detta?</a></p>
...
const TagEditor: React.FC<{ tag: Tag, onSave: (tag: Tag) => void, onCancel: () => void }> = ({ tag, onSave, onCancel }) => {
    ...
    const isCircle = isStamp && currentTag.shape === 'circle';
    ...
    if (isStamp) {
        if (isCircle) {
            stampClasses += ' rounded-full aspect-square justify-center text-center ';
        } else {
        ...
        if (currentTag.border === 'solid') {
            ...
            stampStyles.borderColor = currentTag.textColor;
            ...
            {/* Live Preview */}
             <div className="p-4 rounded-lg bg-slate-200 dark:bg-slate-900/50 flex items-center justify-center min-h-[80px] border border-slate-300 dark:border-slate-600 border-dashed">
                <div
                    ...
                    className={`
                        inline-flex items-center shadow-lg uppercase tracking-wider
                        ${currentTag.url ? 'p-1' : (isCircle ? 'p-4' : 'px-4 py-2')}
                ...
                {isStamp ? (
                    <>
                        <div>
                            ...
                            <StyledSelect value={currentTag.shape || 'rectangle'} onChange={e => setCurrentTag({...currentTag, shape: e.target.value as Tag['shape']})}>
