...
export const DisplayScreenEditorScreen: React.FC<DisplayScreenEditorScreenProps> = ({ screen: initialScreen, initialPostToEdit, onUpdateOrganization, userRole }) => {
    ...
    const [postToDownloadAsImage, setPostToDownloadAsImage] = useState<DisplayPost | null>(null);
    ...
    const handleSavePost = async () => {
        ...
        try {
            ...
            if (suggestionOriginId) {
                delete postWithStorageUrls.suggestionOriginId;
            }
            ...
            let updatedSuggestedPosts;
            if (suggestionOriginId) {
                updatedSuggestedPosts = (organization.suggestedPosts || []).map(s =>
                    s.id === suggestionOriginId ? { ...s, status: 'edited-and-published' as const, finalPostId: finalPost.id } : s
                );
            }
    ...
    return (
        <div className="w-full max-w-full mx-auto animate-fade-in pb-12 relative">
            ...
            />
            {postToDownloadAsImage && (
                <StaticPostDownloaderModal
                    post={postToDownloadAsImage}
                    screen={screen}
                    organization={organization}
                    onClose={() => setPostToDownloadAsImage(null)}
                />
            )}
            <CreatePostModal
