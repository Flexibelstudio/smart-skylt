...
export const DisplayScreenEditorScreen: React.FC<DisplayScreenEditorScreenProps> = ({ screen: initialScreen, initialPostToEdit, onUpdateOrganization, userRole, onDirtyChange }) => {
    ...
    const isDirty = useMemo(() => {
        return editingPost !== null && JSON.stringify(editingPost) !== JSON.stringify(originalPost);
    ...
    return (
        <div className="w-full max-w-full mx-auto animate-fade-in pb-12 relative">
                    ...
                    <ControlPanel
                        ...
                        onEditPost={(post) => {
                            ...
                            if (cleanPost.layout === 'image-fullscreen' || cleanPost.layout === 'video-fullscreen') {
                                if (cleanPost.headlineFontScale === undefined || cleanPost.headlineFontScale === 5.5 || cleanPost.headlineFontScale === 3.6 || cleanPost.headlineFontScale === 5.0 || cleanPost.headlineFontScale === 8.5 || cleanPost.headlineFontScale === 10.0) {
                                    cleanPost.headlineFontScale = isPortrait ? 8.5 : 5.5; // Synkat till snabbinläggets exakta perfektion
                                }
                                if (cleanPost.bodyFontScale === undefined || cleanPost.bodyFontScale === 3.8 || cleanPost.bodyFontScale === 2.5 || cleanPost.bodyFontScale === 2.4 || cleanPost.bodyFontScale === 4.2 || cleanPost.bodyFontScale === 4.8) {
                                    cleanPost.bodyFontScale = isPortrait ? 4.2 : 3.0; // Synkat till snabbinläggets exakta perfektion
                                }
                            }
                            ...
                            if (cleanPost.isExpressPost) {
                                ...
                                if (cleanPost.layout === 'image-fullscreen' || cleanPost.layout === 'video-fullscreen') {
                                    ...
                                    if (cleanPost.headlinePositionY === undefined) cleanPost.headlinePositionY = isPortrait ? 70 : 65;
                                    if (cleanPost.headlineWidth === undefined) cleanPost.headlineWidth = isPortrait ? 90 : 80;
                                    if (cleanPost.headlineFontScale === undefined || cleanPost.headlineFontScale === 8.5 || cleanPost.headlineFontScale === 10.0) cleanPost.headlineFontScale = isPortrait ? 8.5 : 5.5;
                                    ...
                                    if (cleanPost.bodyFontScale === undefined || cleanPost.bodyFontScale === 4.2 || cleanPost.bodyFontScale === 4.8) cleanPost.bodyFontScale = isPortrait ? 4.2 : 3.0;
            ...
            <ConfirmDialog
                ...
                title="Lämna redigeraren?"
                confirmText="Ja, lämna"
                cancelText="Minska ändringar"
            >
                Du har ändringar på det här inlägget som inte har sparats. Är du säker på att du vill lämna och förlora dina ändringar?
            </ConfirmDialog>
            ...
            {isExpressPublishOpen && createPortal(
                <div className="fixed inset-0 z-50 overflow-y-auto flex items-start justify-center p-4 sm:p-6 md:p-10">
                    <div 
                        className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity cursor-pointer"
                        onClick={() => setIsExpressPublishOpen(false)}
                    />
                    <div className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200/85 dark:border-slate-850 w-full max-w-6xl z-10 my-4 md:my-8 transform transition-all">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 rounded-t-3xl">
                            ...
                            <button
                                onClick={() => setIsExpressPublishOpen(false)}
                                className="p-1 px-2.5 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors font-bold text-lg cursor-pointer"
                                aria-label="Stäng"
                            >
                                ✕
                            </button>
                        </div>
                        <div className="p-6 w-full max-h-[calc(100vh-160px)] overflow-y-auto">
                            <ExpressPublishTab
                                organization={organization}
                                onUpdateOrganization={onUpdateOrganization}
