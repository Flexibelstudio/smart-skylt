...
export const MediaGalleryTab: React.FC<MediaGalleryTabProps> = ({ organization, onUpdateOrganization }) => {
    ...
    const [uploadProgress, setUploadProgress] = useState<{ total: number; current: number; percent: number } | null>(null);
    ...
    const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Filter out files that are too big (> 20MB)
        const validFiles = files.filter(file => {
            if (file.size > 20 * 1024 * 1024) {
                showToast({ message: `Filen "${file.name}" är för stor (max 20MB).`, type: 'error' });
                return false;
            }
            return true;
        });
        ...
        setIsUploading(true);
        setUploadProgress({ total: validFiles.length, current: 0, percent: 0 });
            ...
            for (let i = 0; i < validFiles.length; i++) {
                ...
                try {
                    const { url } = await uploadMediaForGallery(organization.id, file, (progress) => {
                        setUploadProgress(prev => prev ? { ...prev, percent: Math.round(progress) } : null);
                    });
                    ...
                    console.error(`Kunde inte ladda upp "${file.name}":`, err);
                    showToast({ message: `Kunde inte ladda upp "${file.name}".`, type: 'error' });
                }
            }

            if (uploadedItems.length > 0) {
                const updatedLibrary = [...uploadedItems, ...mediaLibrary];
                await onUpdateOrganization(organization.id, { mediaLibrary: updatedLibrary });
                showToast({ message: `${uploadedItems.length} fil(er) har laddats upp!`, type: 'success' });
            }
        } catch (error) {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            >
                ...
                {uploadProgress && (
                    <div className="mb-6 p-4 bg-purple-50 dark:bg-purple-950/20 rounded-xl border border-purple-100 dark:border-purple-800/40 space-y-2 animate-fade-in text-slate-800 dark:text-slate-100">
                        <div className="flex justify-between items-center text-xs font-bold text-purple-700 dark:text-purple-400 uppercase tracking-tight">
                            <span>Laddar upp filer ({uploadProgress.current}/{uploadProgress.total})</span>
                            <span>{uploadProgress.percent}%</span>
                        </div>
                        <div className="w-full bg-slate-200 dark:bg-slate-755 rounded-full h-2 overflow-hidden">
                            <div 
                                className="bg-purple-600 h-2 rounded-full transition-all duration-300" 
                                style={{ width: `${((uploadProgress.current - 1) / uploadProgress.total) * 100 + (uploadProgress.percent / uploadProgress.total)}%` }}
                            ></div>
                        </div>
                    </div>
                )}
