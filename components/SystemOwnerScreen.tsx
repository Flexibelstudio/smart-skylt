...
import { Organization, SystemSettings } from '../types';
import { getSystemSettings, updateSystemSettings, createSystemAnnouncement } from '../services/firebaseService';
import { MonitorIcon, PencilIcon } from './icons';
...
const InvoiceLineItem: React.FC<{
    ...
    pricePerUnit: number;
    discount?: number;
}> = ({ icon, label, quantity, pricePerUnit, discount = 0 }) => {
        ...
        <div className="flex justify-between items-center text-sm py-2 border-b border-slate-200 dark:border-gray-700">
            ...
            <div className="flex items-center gap-6">
                 <span className="text-gray-600 dark:text-gray-400 text-right">
                    {quantity} st á {pricePerUnit} kr
                    {discount > 0 && (
                        <span className="block text-red-500 text-xs">
                            - {discount}% rabatt (-{discountAmount.toLocaleString('sv-SE')} kr)
                        </span>
                    )}
                </span>
                <span className="font-bold text-gray-900 dark:text-white w-24 text-right">{finalPrice.toLocaleString('sv-SE')} kr</span>
            </div>
        </div>
    );
};
...
const OrganizationCard: React.FC<{
    ...
    const billableScreensCount = physicalScreens.length;
    ...
    const additionalScreens = Math.max(0, billableScreensCount - 1);
    const additionalScreenCost = additionalScreens * priceAdditional;

    const totalCostBeforeDiscount = basePrice + additionalScreenCost;
    const discountAmount = totalCostBeforeDiscount * (discount / 100);
    const grandTotal = totalCostBeforeDiscount - discountAmount;
                ...
                <div className="flex-grow">
                    <p className="text-xl font-bold text-gray-900 dark:text-white">{org.name}</p>
                     <div className="mt-2">
                        ...
                        <p className="text-3xl font-extrabold text-primary">{grandTotal.toLocaleString('sv-SE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kr</p>
                    </div>
                </div>
            ...
            {isExpanded && (
                <div className="bg-slate-50 dark:bg-gray-800/50 p-4 border-t border-slate-200 dark:border-gray-700 space-y-2 animate-fade-in">
                    <InvoiceLineItem
                        ...
                        label="Grundpris (inkl. 1 skyltfönster)"
                        quantity={1}
                        pricePerUnit={basePrice}
                    />
                    {additionalScreens > 0 && (
                        <InvoiceLineItem
                            icon={<MonitorIcon className="h-5 w-5 text-blue-400" />}
                            label="Ytterligare skyltfönster"
                            quantity={additionalScreens}
                            pricePerUnit={priceAdditional}
                        />
                    )}
                    {discount > 0 && (
                        <div className="flex justify-between items-center text-sm py-2 border-b border-slate-200 dark:border-gray-700">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-semibold text-red-500">Organisationsrabatt</span>
                            </div>
                            <div className="flex items-center gap-6">
                                <span className="text-red-500 text-right">
                                    {discount}% på {totalCostBeforeDiscount.toLocaleString('sv-SE')} kr
                                </span>
                                <span className="font-bold text-red-500 w-24 text-right">-{discountAmount.toLocaleString('sv-SE')} kr</span>
                            </div>
                        </div>
                    )}
                     <div className="flex justify-end pt-2">
                        <div className="flex items-baseline gap-4">
                            ...
                            <span className="font-extrabold text-lg text-gray-900 dark:text-white">{grandTotal.toLocaleString('sv-SE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kr</span>
                        </div>
                     </div>
