...
const ImageWithFallback: React.FC<{ src?: string; alt: string; className: string; }> = ({ src, alt, className }) => {
    ...
    return <img src={imgSrc} onError={handleError} alt={alt} className={`${className} ${imgSrc === PLACEHOLDER_URL ? 'object-contain p-4 bg-slate-200 dark:bg-slate-700' : ''}`} />;
};
...
const VideoWithFallback: React.FC<React.ComponentProps<'video'> & { src?: string }> = ({ src, ...props }) => {
    ...
    if (!videoSrc) {
        return <img src={PLACEHOLDER_URL} alt="Missing video" className={props.className + ' object-contain p-4 bg-slate-200 dark:bg-slate-700'} />;
    }
...
const TextContent: React.FC<TextContentProps> = ({ post, mode, onUpdateTextPosition, isTextDraggable, cycleCount = 0, organization, aspectRatio, isForDownload }) => {
        ...
        <div className="w-full h-full relative overflow-hidden" style={{ backgroundColor }}>
            {post.layout.includes('image') && post.imageUrl && (
             ...
             {post.layout.includes('video') && post.videoUrl && (
                <VideoWithFallback ref={videoRef} src={post.videoUrl} autoPlay muted playsInline className={mediaBaseClasses} style={mediaStyle} />
            )}
            ...
            {(post.headline || post.body) && <TextContent post={post} mode={mode} onUpdateTextPosition={onUpdateTextPosition} isTextDraggable={isTextDraggable} cycleCount={cycleCount} organization={organization} aspectRatio={aspectRatio} isForDownload={isForDownload} />}
