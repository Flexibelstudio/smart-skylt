...
import { DisplayPost, Tag, SubImage, SubImageConfig, ContentPosition, TagPositionOverride, CollageItem, Organization } from '../types';
...
const TextContent: React.FC<TextContentProps> = ({ post, mode, onUpdateTextPosition, isTextDraggable, cycleCount = 0, organization }) => {
    ...
    const getInitialXY = () => {
        if (post.textPositionX !== undefined && post.textPositionY !== undefined) {
            return { x: post.textPositionX, y: post.textPositionY };
        }
        const pos = post.textPosition || 'middle-center';
        let y = 50;
        if (pos.includes('top')) y = 15;
        if (pos.includes('bottom')) y = 85;

        let x = 50;
        if (pos.includes('left')) x = 25;
        if (pos.includes('right')) x = 75;

        return { x, y };
    };

    const { x: posX, y: posY } = getInitialXY();
...
const DraggableTag: React.FC<DraggableTagProps> = ({ tag, override, mode, onUpdatePosition }) => {
    ...
    const tagContent = (
        <div
            ...
            className={`
                inline-flex items-center rounded-lg shadow-lg uppercase tracking-wider z-20
                ${tag.url ? (isPreviewMode(mode) ? 'p-0.5' : 'p-1') : (isPreviewMode(mode) ? 'px-2 py-1' : 'px-4 py-2')}
        ...
        >
            {tag.url ? (
                 <div className={`flex items-center ${isPreviewMode(mode) ? 'gap-1' : 'gap-2'}`}>
                    <span>{tag.text || "Taggtext"}</span>
                    <div className="bg-white p-0.5 rounded-sm">
                        <QrCodeComponent url={tag.url} size={isPreviewMode(mode) ? 16 : 24} color={{dark: '#000', light: '#fff'}} />
                    </div>
                </div>
            ) : (
                tag.text || "Taggtext"
    ...
    return (
        <div className="w-full h-full relative overflow-hidden" style={{ backgroundColor }}>
            ...
            {post.layout.includes('image') && post.imageUrl && (
                <img src={post.imageUrl} alt={post.headline || 'Post image'} className={`absolute inset-0 w-full h-full object-cover z-1 ${post.imageEffect === 'ken-burns-slow' ? 'animate-ken-burns-slow' : ''} ${post.imageEffect === 'ken-burns-fast' ? 'animate-ken-burns-fast' : ''}`} />
            )}
             {post.layout.includes('video') && post.videoUrl && (
                <video ref={videoRef} src={post.videoUrl} autoPlay muted playsInline className="absolute inset-0 w-full h-full object-cover z-1" />
            )}
            ...
            {(post.headline || post.body) && <TextContent post={post} mode={mode} onUpdateTextPosition={onUpdateTextPosition} isTextDraggable={isTextDraggable} cycleCount={cycleCount} organization={organization} />}
