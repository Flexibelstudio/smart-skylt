...
import { DisplayPost, Organization, DisplayScreen, Tag, SubImage, SubImageConfig, CollageItem, MediaItem, TagColorOverride } from '../../types';
import { PrimaryButton, SecondaryButton } from '../Buttons';
import { StyledInput, StyledSelect, FontSelector } from '../Forms';
import { ChevronDownIcon, ToggleSwitch, SparklesIcon, TrashIcon, CompactToggleSwitch, InstagramIcon, LoadingSpinnerIcon } from '../icons';
import { 
...
export const PostEditor: React.FC<PostEditorProps> = ({ post, organization, aspectRatio, onPostChange, onSave, onCancel, isSaving, onUpdateOrganization }) => {
    ...
    const handleMediaSelect = (item: MediaItem) => {
        ...
        if (target === 'main') {
            if (item.type === 'image') {
                onPostChange({ ...post, imageUrl: item.url, videoUrl: undefined, isAiGeneratedImage: item.createdBy === 'ai', isAiGeneratedVideo: false });
            } else { // video
                onPostChange({ ...post, videoUrl: item.url, imageUrl: undefined, isAiGeneratedImage: false, isAiGeneratedVideo: item.createdBy === 'ai' });
            }
        } else if (target === 'sub') {
            if (item.type === 'image') {
                const newSubImage: SubImage = { id: `subimg-${Date.now()}`, imageUrl: item.url };
                onPostChange({ ...post, subImages: [...(post.subImages || []), newSubImage] });
            } else {
    ...
    const handleGenerateImageFromPrompt = async () => {
        ...
        try {
            ...
            onPostChange({ ...post, imageUrl: newImageUrl, isAiGeneratedImage: true });
        ...
        <div className="space-y-6 bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm animate-fade-in">
            ...
            {post.layout !== 'instagram' && post.layout !== 'instagram-latest' && post.layout !== 'instagram-stories' ? (
                <>
                    ...
                    <AccordionSection title="Text & Utseende" defaultOpen={true}>
                        <div className="space-y-4">
                            <div>
