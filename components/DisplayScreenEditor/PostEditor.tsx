...
export const PostEditor: React.FC<PostEditorProps> = ({ post, organization, aspectRatio, onPostChange, onSave, onCancel, isSaving, onUpdateOrganization, userRole }) => {
    ...
    const [aiStudioLayout, setAiStudioLayout] = useState('');
    ...
    useEffect(() => {
        ...
        setAiJustGenerated(false); // Reset on new post
    }, [post?.id]);
    ...
    useEffect(() => {
        if (post.aiImagePrompt) {
            setAiCombinedPrompt(post.aiImagePrompt);
        } else if (post.id.startsWith('new-')) {
            setAiCombinedPrompt(`${post.headline || ''}\n${post.body || ''}`.trim());
        } else {
            setAiCombinedPrompt('');
        }
    ...
    const handleGenerateCompletePost = async () => {
        ...
        try {
            ...
            // Nollställ AI-promptfältet efter generering för att undvika förvirring — redigering sker i "Text & Utseende".
            setAiCombinedPrompt('');
    ...
    const handleGenerateImageFromPrompt = async () => {
        if (!post.aiImagePrompt) return;
        setAiLoading('generate');
        try {
            const newImageUrl = await generateDisplayPostImage(post.aiImagePrompt, aspectRatio);
            updatePostAndSaveHistory({ ...post, imageUrl: newImageUrl, isAiGeneratedImage: true });
            showToast({ message: 'Bilden har genererats!', type: 'success' });
        } catch (error) {
    ...
    const layoutOptionsForAI = [
        { label: 'AI Väljer', value: '' },
        { label: 'Bara Text', value: 'text-only' },
        { label: 'Helskärmsbild', value: 'image-fullscreen' },
        { label: isPortrait ? 'Bild över' : 'Bild vänster', value: 'image-left' },
        { label: isPortrait ? 'Bild under' : 'Bild höger', value: 'image-right' },
    ];
        ...
        <div className="space-y-6 bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm animate-fade-in">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 p-4 sm:p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <div className="space-y-4">
                    <AIIdeaGenerator 
                        initialInput={aiCombinedPrompt}
                        onIdeaSelect={handleIdeaSelect}
                        isLoading={!!aiLoading}
                        organization={organization}
                    />

                    <p className="text-sm text-slate-600 dark:text-slate-400 pt-4 border-t border-slate-200 dark:border-slate-700">
                        Eller, beskriv ditt inlägg med några ord så skapar AI:n ett komplett förslag med text, bild och design.
                    </p>
                    <textarea
                        rows={3}
                        value={aiCombinedPrompt}
                        onChange={e => {
                            setAiCombinedPrompt(e.target.value);
                            if (aiJustGenerated) {
                        ...
                        disabled={!!aiLoading}
                    />
                    {!aiCombinedPrompt && !aiJustGenerated && (
                        <p className="text-xs text-slate-500 dark:text-slate-400 -mt-2">
                            Tips: Beskriv kort vad du vill skapa — AI:n fixar resten!
                        </p>
                    )}
                    
                    {/* // Finjusterad spacing, ikonfärg och hover-effekt för att ge Bildinställningar en mer polerad, taktil känsla. */}
                    <details
                        ref={sectionRef}
                        // Highlight av Bildinställningar triggas endast när användaren kommer från AI-flödet (vald AI-idé).
                        className={`group mt-6 rounded-lg transition-shadow`}
                    >
                        <summary className="cursor-pointer list-none bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/40 hover:shadow-sm p-3 rounded-lg transition-all duration-200">
                            <div className="flex items-center justify-between">
                                <div className="flex items-baseline gap-3">
                                    <PhotoIcon className="h-6 w-6 text-blue-500 dark:text-blue-500" />
                                    <div>
                                        <span className="font-semibold text-base text-slate-800 dark:text-slate-100">Bildinställningar</span>
                                        <p className="text-xs text-slate-500 dark:text-slate-400 group-open:hidden mt-1">
                                            Används när AI skapat ett bildförslag – justera bildförhållande, beskärning och position.
                                        </p>
                                    </div>
                                </div>
                                <ChevronDownIcon className="h-5 w-5 text-slate-500 dark:text-slate-400 transition-transform group-open:rotate-180" />
                            </div>
                        </summary>
                        <div className="mt-4 pl-5 space-y-4 border-l-2 border-slate-200 dark:border-slate-700 group-open:animate-fade-in">
                            <p className="text-sm text-slate-500 dark:text-slate-400">Finslipa din bildidé genom att välja stil, färger och känsla.</p>
                            <AiStudioModifierGroup label="Layout" options={layoutOptionsForAI} selectedValue={aiStudioLayout} onSelect={value => setAiStudioLayout(value)} />
                            ...
                            <AiStudioModifierGroup label="Komposition" options={compositionOptions} selectedValue={aiStudioState.composition} onSelect={value => setAiStudioState(s => ({ ...s, composition: value }))} />
                            <AiStudioModifierGroup label="Belysning" options={lightingOptions} selectedValue={aiStudioState.lighting} onSelect={value => setAiStudioState(s => ({ ...s, lighting: value }))} />
                    ...
                    <AccordionSection title="Media" disabled={isSharedPost}>
                        {post.layout === 'collage' ? (
                            ...
                            <div className="space-y-4">
                                ...
                                {!post.imageUrl && !post.videoUrl && post.aiImagePrompt && (
                                    <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800 space-y-2">
                                        <h4 className="font-bold text-purple-800 dark:text-purple-300 flex items-center gap-2"><SparklesIcon className="h-5 w-5"/> AI Bildförslag</h4>
                                        <p className="text-sm text-purple-700 dark:text-purple-300/80 italic">
                                            "{post.aiImagePrompt}"
                                        </p>
                                        <PrimaryButton 
