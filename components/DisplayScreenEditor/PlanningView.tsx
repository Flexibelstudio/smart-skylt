...
const EventEditorModal: React.FC<{
    ...
    return ReactDOM.createPortal(
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-slate-800 rounded-xl p-6 sm:p-8 w-full max-w-md text-white shadow-2xl border border-slate-700 animate-fade-in" onClick={e => e.stopPropagation()}>
                <h2 className="text-2xl font-bold mb-4">{event ? 'Redigera händelse' : 'Skapa ny händelse'}</h2>
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-400 mb-1">Namn</label>
                        <input type="text" value={name} onChange={e => setName(e.target.value)} className="w-full bg-slate-900 p-2 rounded-md border border-slate-600" />
                    </div>
                    <div className="grid grid-cols-3 gap-4">
                        <div className="col-span-2">
                            <label className="block text-sm font-medium text-slate-400 mb-1">Datum</label>
                            <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full bg-slate-900 p-2 rounded-md border border-slate-600" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-400 mb-1">Ikon</label>
                            <input type="text" value={icon} onChange={e => setIcon(e.target.value)} maxLength={2} className="w-full bg-slate-900 p-2 rounded-md border border-slate-600 text-center text-xl" />
                        </div>
                    </div>
                </div>
                <div className="flex justify-end gap-4 mt-6">
                    <button onClick={onClose} className="bg-slate-600 hover:bg-slate-500 font-semibold py-2 px-4 rounded-lg">Avbryt</button>
...
interface PlanningViewProps {
    ...
    onGetCampaignIdeas: (eventName: string) => void;
...
export const PlanningView: React.FC<PlanningViewProps> = ({ screen, posts, organization, onUpdateOrganization, onGetCampaignIdeas, isAIAssistantEnabled, onUpdatePosts }) => {
    ...
    const scheduledPosts = useMemo(() => localPosts.filter(p => {
        ...
        const postStart = new Date(p.startDate); const postEnd = p.endDate ? new Date(p.endDate) : timelineRange.end;
        return postStart <= timelineRange.end && postEnd >= timelineRange.start;
    }).sort((a, b) => new Date(a.startDate!).getTime() - new Date(b.startDate!).getTime()), [localPosts, timelineRange]);
    ...
    const postLayout = useMemo(() => {
        ...
        scheduledPosts.forEach(post => {
            ...
            const end = post.endDate ? new Date(post.endDate) : timelineRange.end;
            let placed = false;
    ...
    const handleMouseDown = (e: React.MouseEvent, post: DisplayPost, type: 'move' | 'resize-start' | 'resize-end') => {
        ...
        setDraggedPostInfo({ post, type, initialX: e.clientX, initialStart: new Date(post.startDate), initialEnd: post.endDate ? new Date(post.endDate) : addDays(new Date(post.startDate), 1) });
    };
                    ...
                    {(organization.customEvents || []).map(event => (
                        <div key={event.id} className="bg-slate-50 dark:bg-slate-700/50 p-3 rounded-lg flex justify-between items-center border border-slate-200 dark:border-slate-700">
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={() => onGetCampaignIdeas(event.name)} disabled={!isAIAssistantEnabled} title="Få AI-idéer" className="p-2 rounded-full bg-yellow-400/10 hover:bg-yellow-400/20 text-yellow-400 hover:text-yellow-300 transition-colors disabled:opacity-50">
                    ...
                    <div className="relative min-w-[1200px] select-none">
                        ...
                        <div className="relative bg-slate-50 dark:bg-slate-800/50 rounded-lg" style={{ height: `${Math.max(1, postLayout.laneCount) * 40 + 40}px` }}>
                            ...
                            {postLayout.layout.map(({ post, lane }) => {
                                const startDay = daysIntoRange(new Date(post.startDate!));
                                const endDay = daysIntoRange(post.endDate ? new Date(post.endDate) : timelineRange.end);
