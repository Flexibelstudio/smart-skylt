...
const getCollageSlotCount = (layout?: string): number => {
    switch (layout) {
        ...
        case 'landscape-3-vert':
        case 'landscape-1-top-2-bottom':
        case 'portrait-1-2':
        case 'portrait-3-vert':
        case 'portrait-3-horiz':
        case 'portrait-1-top-2-bottom':
            return 3;
        case 'landscape-4-grid':
        case 'landscape-1-top-3-bottom':
        case 'portrait-4-grid':
        case 'portrait-1-top-3-bottom':
            return 4;
        case 'landscape-6-grid':
        case 'portrait-6-grid':
            return 6;
        ...
        <div className="space-y-6">
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <h4 className="font-extrabold text-lg text-slate-800 dark:text-slate-100 tracking-tight">Välj media</h4>
                </div>

                {/* Elegant Segmented Pill Buttons */}
                <div className="flex p-1 bg-slate-100 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-2xl gap-1">
                    {[
                        { id: 'upload' as const, label: 'Eget material', desc: 'Ladda upp / Galleri' },
                        { id: 'ai-image' as const, label: 'Generera bild', desc: 'Skapa med AI' },
                        { id: 'ai-video' as const, label: 'Animera video', desc: 'Rörliga bakgrunder' }
                    ].map(tab => {
                        const isTabActive = activeMediaTab === tab.id;
                        return (
                            <button
                                key={tab.id}
                                type="button"
                                ...
                                className={`flex-1 py-2 px-2.5 rounded-xl transition-all text-center cursor-pointer ${
                                    isTabActive 
                                        ? 'bg-white dark:bg-slate-850 text-primary shadow-sm font-extrabold' 
                                        : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'
                                }`}
                            >
                                <span className="block text-xs font-bold whitespace-nowrap leading-tight">{tab.label}</span>
                                <span className="text-[9px] text-slate-400 dark:text-slate-500 block mt-0.5 leading-none font-normal">{tab.desc}</span>
                            </button>
                        );
                    })}
                </div>
                ...
                {activeMediaTab === 'upload' && (
                    <div className="p-6 rounded-2xl bg-white dark:bg-slate-800/20 border border-slate-200 dark:border-slate-700/60 space-y-4 animate-fade-in">
                        <div 
                            className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-705 rounded-xl p-8 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors cursor-pointer group" 
                            onClick={() => document.getElementById('file-upload-step2')?.click()}
                        >
                            <div className="p-3.5 bg-primary/10 rounded-full text-primary mb-3.5 group-hover:scale-110 transition-transform">
                                <PhotoIcon className="w-8 h-8" />
                            </div>
                            <span className="font-extrabold text-sm text-slate-700 dark:text-slate-200">Klicka för att ladda upp fil</span>
                            <span className="text-xs text-slate-400 dark:text-slate-500 mt-1">Stöder bilder och videor (MP4)</span>
                        </div>
                        
                        <div className="flex items-center justify-between gap-4 pt-2">
                            <span className="text-xs font-bold text-slate-400 uppercase tracking-tight">Tidigare uppladdningar?</span>
                ...
                {/* Tab: AI Image Generation */}
                {activeMediaTab === 'ai-image' && (
                    <div className="p-5 rounded-2xl bg-purple-50/10 dark:bg-purple-950/20 border border-purple-200/50 dark:border-purple-800/40 space-y-4 shadow-sm animate-fade-in">
                        <div>
                            <span className="text-[10px] uppercase tracking-wider font-extrabold text-purple-600 dark:text-purple-400 block mb-1">Skapa fotorealistiska bilder</span>
                            <h5 className="font-extrabold text-sm text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                                <span className="text-base">✨</span> Skapa spännande AI-grafik
                            </h5>
                        </div>
                        ...
                        <PrimaryButton 
                            ...
                            className="w-full justify-center bg-purple-600 hover:bg-purple-500 shadow-md shadow-purple-500/20 py-2.5 rounded-xl cursor-pointer"
                        >
                ...
                {activeMediaTab === 'ai-video' && (
                    <div className="p-5 rounded-2xl bg-indigo-50/10 dark:bg-indigo-950/20 border border-indigo-200/50 dark:border-indigo-800/40 space-y-4 shadow-sm animate-fade-in">
                        <div>
                            <span className="text-[10px] uppercase tracking-wider font-extrabold text-indigo-600 dark:text-indigo-400 block mb-1">Gör dina inlägg dynamiska</span>
                            <h5 className="font-extrabold text-sm text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                                <VideoCameraIcon className="w-5 h-5 text-indigo-500"/> Skapa dynamisk video/animering
                            </h5>
                        </div>
                        
                        {isVideoGenerating ? (
                            <div className="flex items-center gap-3 p-4 bg-indigo-650 dark:bg-indigo-700/80 rounded-xl text-white border border-indigo-500 shadow-lg animate-pulse">
                            ...
                            <>
                                {post.imageUrl && (
                                    <div className="space-y-2 mb-1 p-3.5 bg-indigo-50/50 dark:bg-indigo-900/40 rounded-xl border border-indigo-250/20 dark:border-indigo-700/30">
                                        <div className="flex items-center gap-2">
                                            <input
                                                type="checkbox"
                                                id="useImageForVideo"
                                ...
                                <textarea
                                    ...
                                    rows={3}
                                    className="w-full bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-primary focus:border-primary transition-all text-sm text-slate-900 dark:text-white placeholder:text-slate-400"
                                    disabled={!!aiLoading || cooldown > 0}
                                />
                                <PrimaryButton 
                                    ...
                                    className="w-full justify-center bg-indigo-600 hover:bg-indigo-500 shadow-md shadow-indigo-500/20 py-2.5 rounded-xl cursor-pointer"
                                >
                ...
                {isImageGenerating && (
                    <div className="flex items-center gap-3 p-4 bg-purple-600 dark:bg-purple-700 rounded-xl text-white border border-purple-500 shadow-md animate-pulse">
                        ...
                        <div>
                            ...
                            <p className="text-xs opacity-80 mt-1">Bilden skapas utifrån dina instruktioner.</p>
                        </div>
                    </div>
                )}

                {uploadProgress !== null && (
                    <div className="w-full bg-slate-200 dark:bg-slate-750 rounded-full h-2 mt-2 overflow-hidden">
                        <div className="bg-primary h-2 rounded-full transition-all duration-350" style={{ width: `${uploadProgress}%` }}></div>
                    </div>
                )}

                {(post.imageUrl || post.videoUrl) && (
                    <div className="mt-4 space-y-4">
                        <div className="relative w-48 group overflow-hidden rounded-xl shadow-md border border-slate-200 dark:border-slate-700">
                            {post.imageUrl && <img src={post.imageUrl} className="w-full" alt="Media" />}
                            {post.videoUrl && <video src={post.videoUrl} className="w-full" autoPlay muted loop playsInline />}
                            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center p-2 rounded-xl">
                                <button onClick={() => onPostChange({ ...post, imageUrl: undefined, videoUrl: undefined })} disabled={!!aiLoading} className="bg-red-650 hover:bg-red-500 text-white font-bold py-1 px-4.5 rounded-full shadow-lg text-xs cursor-pointer">Ta bort</button>
                            </div>
                        </div>
                        <div className="flex flex-wrap items-center gap-4">
                            {post.imageUrl && (
                                <button type="button" onClick={() => setIsAiEditorOpen(true)} className="flex items-center gap-1 text-sm font-semibold text-purple-600 dark:text-purple-400 hover:underline disabled:opacity-50 cursor-pointer" disabled={!!aiLoading || cooldown > 0}>
...
const CollageMediaEditor: React.FC<{
    ...
    const handleBatchUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Fill empty slots first, then overwrite if needed
        for (let i = 0; i < slotCount && fileIndex < files.length; i++) {
            ...
            try {
                ...
                const newItem: CollageItem = {
                    id: `item-${Date.now()}-${i}`,
                    type: isVideo ? 'video' : 'image',
                    imageUrl: isVideo ? undefined : dataUrl,
                ...
                console.error("Failed to read file in batch upload", error);
            }
        }
        
        onPostChange({ ...post, layout: 'collage', collageItems: updatedItems });
        showToast({ message: `${fileIndex} filer uppladdade till collaget.`, type: 'success' });
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <div>
                    ...
                    />
                    <SecondaryButton onClick={() => document.getElementById('batch-upload-input')?.click()} className="py-1.5 px-3 text-xs">
                        <PhotoIcon className="w-4 h-4 mr-1" /> Fyll automatiskt (Ladda upp flera)
                    </SecondaryButton>
                </div>
            </div>
...
export const Step3_Atmosphere: React.FC<{
    ...
    // --- Layout: Text Only ---
    if (['text-only', 'webpage'].includes(post.layout)) {
        return (
            <div className="space-y-4">
