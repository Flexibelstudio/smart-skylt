...
import { SparklesIcon, HandThumbUpIcon, ChevronDownIcon } from '../../icons';
...
const openPicker = (e: React.MouseEvent<HTMLInputElement>) => {
    ...
    if (typeof el.showPicker === 'function') {
        try { el.showPicker(); } catch { /* vissa webbläsare tillåter inte anropet */ }
    }
};
...
export const Step4_Publishing: React.FC<{
    ...
    const daysText = selectedDays.length === 0
        ? 'Alla dagar'
        : [1, 2, 3, 4, 5, 6, 0].filter(d => selectedDays.includes(d)).map(d => dayLabels[d]).join(', ');
                ...
                <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                    ...
                    >
                        <div className="flex flex-col">
                            ...
                            <span className={hasScheduleLimits ? "text-xs font-semibold text-indigo-600 dark:text-indigo-400" : "text-xs text-slate-400 dark:text-slate-500"}>
                                {scheduleSummary}
                            </span>
                        </div>
