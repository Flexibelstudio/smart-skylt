...
const colorToneOptions = [
    { label: '🔥 Varma toner', value: 'warm tones' },
    { label: '❄️ Kalla toner', value: 'cool tones' },
    { label: '🍭 Pastell', value: 'pastel colors' },
    { label: '⚡ Hög kontrast', value: 'high contrast and saturation, perfect for storefront displays' },
    { label: '🖤 Monokrom', value: 'monochrome, high-end black and white' },
    { label: '☀️ Ljust & Fräscht', value: 'bright airy light, clean overexposed style' },
    { label: '🌙 Kväll / Neon', value: 'neon cinematic lighting, night atmosphere, vibrant pops' },
    { label: '🍂 Höstmys', value: 'cozy autumn aesthetics, warm rustic earth tones' },
    { label: '❄️ Vinter/Friskt', value: 'cool winter aesthetics, crisp clean snow tones' },
    { label: '🌸 Sommar/Färg', value: 'vibrant colorful summer warm sunlight aesthetics' },
    { label: '💎 Premium', value: 'premium color grading, rich professional colors' },
];

const perspectiveOptions = [
    { label: '🔍 Närbild', value: 'close-up shot' },
    { label: '🚶 Halvkropp', value: 'medium shot' },
    { label: '🗺️ Översikt', value: 'wide shot' },
...
const compositionOptions = [
    ...
    { label: '⚖️ Symmetrisk', value: 'symmetrical' },
    { label: '🌪️ Dynamiskt sned', value: 'dynamic Dutch angle composition' },
];

const styleOptions = [
    { label: '📸 Studiofoto', value: 'studio photography' },
    { label: '👥 Lifestyle', value: 'lifestyle photography' },
    { label: '🍔 Mat & Dryck (Fika/Mat)', value: 'food commercial photography, highly appetizing close-up, food commercial lighting' },
    { label: '🎁 Produktfoto', value: 'product photography' },
    { label: '🍽️ Flatlay', value: 'flatlay photography' },
    { label: '✨ Minimalism', value: 'clean minimalism' },
    { label: '🎬 Cinematisk', value: 'cinematic still' },
    { label: '🎨 Grafisk / 3D-Illustration', value: 'modern 3d graphic illustration, clean vector graphic style, colorful' },
    { label: '🧘 Spa & Wellness', value: 'serene wellness spa photography, natural sunlight, organic textures, calm' },
    { label: '🏃 Aktivitet / Träning', value: 'athletic commercial dynamic sports action photography, energetic lighting' },
];

const storyOptions = [
    { label: '🔄 Före / Efter', value: 'before and after concept' },
    { label: '📈 Steg-för-steg', value: 'step-by-step sequence' },
    { label: '📅 Event / Fokus', value: 'focused event atmosphere, happy vibes' },
    { label: '🛍️ Produkt i fokus', value: 'product line-up in focus' },
    { label: '⚡ Rörelse / Fart', value: 'action shot, motion blur' },
    { label: '🛋️ Stilla / Miljö', value: 'still life, cozy interior space atmosphere' },
];
                ...
                {(post.imageUrl || post.videoUrl) && (
                    <div className="mt-4 space-y-4">
                        ...
                        <div className="bg-slate-100 dark:bg-slate-800/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700 space-y-4">
                            ...
                            {/* Position Controls */}
                            <div>
                                <h5 className="font-semibold text-sm mb-3 text-slate-700 dark:text-slate-300">Justera bildposition</h5>
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => onPostChange({ ...post, mediaPositionX: parseInt(e.target.value) })}
                                    ...
                                    <div>
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => onPostChange({ ...post, mediaPositionY: parseInt(e.target.value) })}
                                            className="w-full h-2 bg-slate-200 dark:bg-slate-600 rounded-lg appearance-none cursor-pointer accent-primary"
                                        />
                                    </div>
                                </div>
...
const CollageMediaEditor: React.FC<{
    ...
    const handleAddText = (index: number) => {
        const newItem: Omit<CollageItem, 'id'> = {
            type: 'text',
        ...
        handleUpdateSlot(index, newItem);
    };
            ...
            <div className="flex justify-between items-center">
                ...
                <div>
                    <input 
                        ...
                        accept="image/*,video/*" 
                        className="hidden" 
                        id="batch-upload-input-media"
                        onChange={handleBatchUpload} 
                    />
                    <SecondaryButton onClick={() => document.getElementById('batch-upload-input-media')?.click()} className="py-1.5 px-3 text-xs">
                        ...
                        <div key={index} className="bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden relative group">
                            ...
                            <div className="aspect-square relative">
                                {isLoading ? (
                                    ...
                                    <>
                                        {item.type === 'text' ? (
                                            <div className="w-full h-full flex flex-col p-4 bg-white dark:bg-slate-800 overflow-y-auto gap-3">
                                                <textarea
                                                    value={item.text || ''}
                                                    onChange={(e) => {
                                                        const updatedItems = [...items];
                                                    ...
                                                    className="w-full p-2 text-sm bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded resize-none flex-grow"
                                                    placeholder="Skriv text..."
                                                />
                                                <div className="grid grid-cols-2 gap-2">
                                                    <div>
                                                        <label className="text-[10px] text-slate-500 uppercase font-bold">Textfärg</label>
                                                        <input 
                                                            type="color" 
                                                            value={item.textColor || '#000000'}
                                                            onChange={(e) => {
                                                                const updatedItems = [...items];
                                                                updatedItems[index] = { ...item, textColor: e.target.value };
                                                                onPostChange({ ...post, layout: 'collage', collageItems: updatedItems });
                                                            }}
                                                    ...
                                                    <div>
                                                        <label className="text-[10px] text-slate-500 uppercase font-bold">Bakgrund</label>
                                                        <input 
                                                            type="color" 
                                                            value={item.backgroundColor || '#ffffff'}
                                                            onChange={(e) => {
                                                                const updatedItems = [...items];
                                                                updatedItems[index] = { ...item, backgroundColor: e.target.value };
                                                                onPostChange({ ...post, layout: 'collage', collageItems: updatedItems });
                                                            }}
                                                            className="w-full h-8 cursor-pointer rounded"
                                                        />
                                                ...
                                                <button onClick={() => handleRemoveItem(index)} className="w-full py-1.5 mt-2 bg-red-100 hover:bg-red-200 text-red-700 rounded text-xs font-bold transition-colors">
                                                    Ta bort textruta
                                                </button>
                                            </div>
                                        ) : (
                                    ...
                                    <div className="w-full h-full flex flex-col items-center justify-center gap-3 p-4 bg-white dark:bg-slate-800/30">
                                        ...
                                        >
                                            <PhotoIcon className="h-4 w-4" /> Ladda upp bild/video
                                        </button>
                                        <button 
