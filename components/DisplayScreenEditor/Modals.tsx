...
import { LightBulbIcon, SparklesIcon, FacebookIcon, VideoCameraIcon, InstagramIcon, LoadingSpinnerIcon, PhotoIcon, ArrowUturnLeftIcon, ArrowUturnRightIcon, DownloadIcon } from '../icons';
...
export const DownloadAssetsModal: React.FC<{
    ...
    const handleDownloadMedia = async (url: string, filename: string) => {
        try {
            ...
            // Try opening in a new tab as a fallback for CORS issues etc.
            window.open(url, '_blank');
            showToast({ message: "Nedladdning misslyckades. Försöker öppna i ny flik.", type: 'info' });
        }
    };
    ...
    const mediaAssets = useMemo(() => {
        const assets: { url: string; type: 'image' | 'video'; filename: string; title: string }[] = [];
        const safeTitle = post.internalTitle?.replace(/[^a-z0-9]/gi, '_').toLowerCase() || 'media';
        
        if (post.imageUrl) assets.push({ url: post.imageUrl, type: 'image', filename: `bild_${safeTitle}.png`, title: 'Huvudbild' });
        if (post.videoUrl) assets.push({ url: post.videoUrl, type: 'video', filename: `video_${safeTitle}.mp4`, title: 'Huvudvideo' });
        
        (post.subImages || []).forEach((img, i) => {
            if(img.imageUrl) assets.push({ url: img.imageUrl, type: 'image', filename: `subbild_${i+1}_${safeTitle}.png`, title: `Karusellbild ${i+1}` });
        });
        
        (post.collageItems || []).forEach((item, i) => {
            if (item?.imageUrl) assets.push({ url: item.imageUrl, type: 'image', filename: `collagebild_${i+1}_${safeTitle}.png`, title: `Collagebild ${i+1}` });
            if (item?.videoUrl) assets.push({ url: item.videoUrl, type: 'video', filename: `collagevideo_${i+1}_${safeTitle}.mp4`, title: `Collagevideo ${i+1}` });
        });
    ...
    return ReactDOM.createPortal(
        <>
            <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
                <div className="bg-white dark:bg-slate-800 rounded-xl p-6 sm:p-8 w-full max-w-2xl text-slate-900 dark:text-white shadow-2xl border border-slate-200 dark:border-slate-700 animate-fade-in flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                    <h2 className="text-2xl font-bold mb-4 flex-shrink-0">Ladda ner material från "{post.internalTitle}"</h2>
                    ...
                    <div className="flex-grow overflow-y-auto pr-2 -mr-2 space-y-6">
                        <div className="p-4 bg-slate-100 dark:bg-slate-900/50 rounded-lg">
                            ...
                            <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">Ladda ner en komplett bild av inlägget, precis som det ser ut på skärmen. Perfekt för delning på sociala medier.</p>
                        ...
                        {hasText && (
                            <div>
                                ...
                                <div className="space-y-3">
                                    {post.headline && (
                                        <div>
                                            <label className="text-sm font-semibold text-slate-500 dark:text-slate-400">Rubrik</label>
                                            <div className="flex gap-2">
                                                <textarea readOnly value={post.headline} rows={2} className="w-full bg-slate-100 dark:bg-slate-900/50 p-2 rounded-lg border border-slate-300 dark:border-slate-600 font-semibold" />
                                                <SecondaryButton onClick={() => handleCopyText(post.headline!)}>Kopiera</SecondaryButton>
                                            </div>
                                        </div>
                                    )}
                                    {post.body && (
                                        <div>
                                            <label className="text-sm font-semibold text-slate-500 dark:text-slate-400">Brödtext</label>
                                            <div className="flex gap-2">
                                                <textarea readOnly value={post.body} rows={4} className="w-full bg-slate-100 dark:bg-slate-900/50 p-2 rounded-lg border border-slate-300 dark:border-slate-600" />
                                                <SecondaryButton onClick={() => handleCopyText(post.body!)}>Kopiera</SecondaryButton>
                                            </div>
                                        </div>
                                    )}
                                </div>
                             ...
                             <div>
                                ...
                                <div className="space-y-2">
                                    {mediaAssets.map((asset, index) => (
                                        <div key={index} className="flex items-center justify-between p-2 bg-slate-100 dark:bg-slate-900/50 rounded-lg">
                                            <div className="flex items-center gap-3">
                                                {asset.type === 'image' ? <PhotoIcon className="h-6 w-6 text-slate-500" /> : <VideoCameraIcon className="h-6 w-6 text-slate-500" />}
                                                <span className="font-semibold text-sm">{asset.title}</span>
                                            </div>
                                            <PrimaryButton onClick={() => handleDownloadMedia(asset.url, asset.filename)}>
