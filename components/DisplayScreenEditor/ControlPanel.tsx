...
import { ToggleSwitch, PencilIcon, TrashIcon, ChevronDownIcon, StarIcon, CampaignIcon, DownloadIcon, SparklesIcon, LinkIcon, LoadingSpinnerIcon, InstagramIcon, ShareIcon, DuplicateIcon, EllipsisVerticalIcon } from '../icons';
...
export const ControlPanel: React.FC<ControlPanelProps> = ({
    ...
    onInitiateCreatePost, onSaveAsTemplate, onCreateFollowUp, generatingFollowUpPostId, onSharePost,
                                    ...
                                    <div className="flex gap-1 flex-shrink-0 mt-2 sm:mt-0 self-end sm:self-auto">
                                        ...
                                        <div className="hidden sm:flex items-center gap-1">
                                            <button onClick={() => onCreateFollowUp(post)} disabled={isSaving || !!generatingFollowUpPostId} className="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-500 hover:text-blue-500 disabled:cursor-not-allowed" title="Skapa uppföljande inlägg">
                                                {generatingFollowUpPostId === post.id ? <LoadingSpinnerIcon className="h-5 w-5" /> : <LinkIcon className="h-5 w-5"/>}
                                            </button>
                                        ...
                                        <div className="sm:hidden relative" ref={openDropdownId === post.id ? dropdownRef : null}>
                                            ...
                                            {openDropdownId === post.id && (
                                                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-md shadow-lg border border-slate-200 dark:border-slate-700 z-10 animate-fade-in">
                                                    <button onClick={() => onCreateFollowUp(post)} disabled={isSaving || !!generatingFollowUpPostId} className="w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center gap-2">
                                                        {generatingFollowUpPostId === post.id ? <LoadingSpinnerIcon className="h-4 w-4" /> : <LinkIcon className="h-4 w-4"/>} Skapa uppföljare
                                                    </button>
