...
import { ToggleSwitch, PencilIcon, TrashIcon, ChevronDownIcon, StarIcon, CampaignIcon, DownloadIcon, SparklesIcon, LinkIcon, LoadingSpinnerIcon, InstagramIcon, ShareIcon, DuplicateIcon } from '../icons';
...
export const ControlPanel: React.FC<ControlPanelProps> = ({
    screen, organization, onUpdateDisplayScreens, onEditPost, onDeletePost, onDownloadPost,
                        ...
                        return (
                            <div key={post.id} draggable onDragStart={() => handleDragStart(index)} onDragOver={handleDragOver} onDrop={() => handleDrop(index)} className="bg-slate-50 dark:bg-slate-700/50 p-3 rounded-lg flex items-center gap-3 border border-slate-200 dark:border-slate-700 cursor-grab active:cursor-grabbing" style={{ opacity: dragIndex === index ? 0.5 : 1 }}>
                                ...
                                <div className="flex gap-1">
                                   ...
                                   {/* <button onClick={() => onDownloadPost(post)} disabled={isSaving} className="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-500 hover:text-green-500" title="Ladda ner som video/bild"><DownloadIcon className="h-5 w-5"/></button> */}
