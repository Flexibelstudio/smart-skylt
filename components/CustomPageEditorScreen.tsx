...
export const CustomPageEditorScreen: React.FC<CustomPageEditorScreenProps> = ({ onSave, onCancel, pageToEdit, onDirtyChange }) => {
    ...
    const isDirty = useMemo(() => {
        const originalTitle = pageToEdit?.title || '';
        const originalTabs = pageToEdit?.tabs || [];
        
        if (!pageToEdit) {
            return title.trim() !== '' || tabs.length > 1 || (tabs[0] && tabs[0].content.trim() !== '') || (tabs[0] && tabs[0].title !== 'Ny Flik');
        }
        
        const hasTitleChanged = title.trim() !== originalTitle.trim();
        const hasTabsChanged = JSON.stringify(tabs) !== JSON.stringify(originalTabs);
        
        return hasTitleChanged || hasTabsChanged;
    }, [title, tabs, pageToEdit]);
        ...
        <div className="w-full max-w-7xl mx-auto space-y-6 pb-28 animate-fade-in">
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-black/80 backdrop-blur-sm p-4 border-t border-slate-200 dark:border-gray-700 z-10">
                <div className="max-w-7xl mx-auto flex justify-end gap-4">
                    <button onClick={handleCancelClick} className="bg-slate-200 dark:bg-gray-600 hover:bg-slate-300 dark:hover:bg-gray-500 text-slate-800 dark:text-white font-bold py-3 px-6 rounded-lg transition-colors">Avbryt</button>
                    <PrimaryButton 
                        onClick={handleSave} 
                        disabled={!isSavable}
                        loading={isProcessing}
            ...
            >
                Du har ändringar på den här sidan som inte har sparats. Är du säker på att du vill lämna och förlora dina ändringar?
            </ConfirmDialog>
        </div>
    );
};