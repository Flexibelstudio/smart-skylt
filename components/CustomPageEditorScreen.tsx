        ...
        <div className="w-full max-w-7xl mx-auto space-y-6 pb-28 animate-fade-in">
            ...
            {aiModalState !== 'closed' && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setAiModalState('closed')}>
                    <div className="bg-gray-800 rounded-xl p-6 sm:p-8 w-full max-w-2xl text-white shadow-2xl border border-gray-700 animate-fade-in" onClick={e => e.stopPropagation()}>
                        {aiModalState === 'choice' && (
                             <div>
                                <h2 className="text-2xl font-bold mb-4">✨ AI-magi</h2>
                                <p className="text-gray-300 mb-6">Vad vill du göra?</p>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <button onClick={handleFormatWithAI} className="bg-gray-700 hover:bg-gray-600 p-6 rounded-lg text-left transition-colors">
                                        <h3 className="font-bold text-lg text-primary">Formatera befintlig text</h3>
                                        <p className="text-sm text-gray-400 mt-1">Strukturera och snygga till texten som redan finns i redigeraren.</p>
                                    </button>
                                    <button onClick={() => setAiModalState('generate')} className="bg-gray-700 hover:bg-gray-600 p-6 rounded-lg text-left transition-colors">
                                        <h3 className="font-bold text-lg text-primary">Skapa ny text från idé</h3>
                                        <p className="text-sm text-gray-400 mt-1">Generera helt nytt innehåll baserat på en kort beskrivning. Ersätter befintlig text.</p>
                                    </button>
                                </div>
                            </div>
                        )}
                        {aiModalState === 'generate' && (
                            <div>
                                ...
                                <p className="text-gray-300 mb-6">Beskriv vad sidan ska handla om, så skriver AI:n ett utkast åt dig.</p>
                                <textarea
                                    value={aiPrompt}
                                    onChange={e => setAiPrompt(e.target.value)}
                                    placeholder="T.ex. 'en välkomstsida för nya medlemmar som förklarar våra öppettider och hur man bokar pass'"
                                    className="w-full h-24 bg-black p-3 rounded-md border border-gray-600"
                                    disabled={isProcessing}
                                />
                                <div className="flex justify-end gap-2 mt-2">
                                    <button onClick={() => setAiModalState('choice')} className="text-gray-400 hover:text-white px-4 py-2">Tillbaka</button>
                                ...
                                {generatedAiText && !isProcessing && (
                                    <div className="mt-4 space-y-4">
                                        ...
                                        <div className="max-h-60 overflow-y-auto bg-black p-4 rounded-md border border-gray-600">
