...
import { ChatMessage, Organization } from '../types';
import { initializeMarketingCoachChat } from '../services/geminiService';
import { LoadingSpinnerIcon, PaperAirplaneIcon } from './icons';
...
export const MarketingCoachBot: React.FC<MarketingCoachBotProps> = ({ onClose, organization }) => {
    ...
    useEffect(() => {
        const initChat = async () => {
            const chatInstance = await initializeMarketingCoachChat(organization);
            setChat(chatInstance);
            setMessages([{
                role: 'model',
                parts: [{ text: "Hej! Jag är din AI Marknadscoach. Hur kan jag hjälpa dig att skapa fantastiskt innehåll idag? Berätta vad du funderar på!" }]
            }]);
            setIsLoading(false);
        };
        initChat();
    ...
    return (
        <div className="fixed bottom-24 right-6 w-[90vw] max-w-md h-[70vh] max-h-[600px] bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col z-50 animate-fade-in">
            <header className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700 flex-shrink-0">
                <h3 className="font-bold text-lg text-slate-900 dark:text-white">AI Marknadscoach</h3>
                ...
                <div className="space-y-4">
                    {messages.map((msg, index) => (
                        <div key={index} className={`flex items-start gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                            {msg.role === 'model' && <div className="w-8 h-8 rounded-full bg-purple-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">AI</div>}
            ...
            <footer className="p-4 border-t border-slate-200 dark:border-slate-700 flex-shrink-0">
                <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                    <textarea
                        ...
                        placeholder="Bolla en idé med coachen..."
                        rows={1}
