...
const isFST = (v: any): v is FirestoreTimestamp => v && (typeof v.seconds === 'number' || typeof v.toDate === 'function');

function parseMaybeDate(value: any, asEndOfDay = false): Date | null {
  if (value === null || value === undefined) return null;

  if (isFST(value)) {
    if (typeof value.toDate === 'function') return value.toDate();
    const ms = (value.seconds ?? 0) * 1000 + Math.floor((value.nanoseconds ?? 0) / 1e6);
    return new Date(ms);
  }
  if (typeof value === 'number') {
    const ms = value > 1e12 ? value : value * 1000; // s → ms
    return new Date(ms);
  }
  ...
  if (typeof value === 'string') {
    const s = value.trim();
    // YYYY-MM-DD → lokal start/slut på dagen
    if (/^\d{4}-\d{2}-\d{2}$/.test(s)) {
      const [y, m, d] = s.split('-').map(Number);
      return asEndOfDay ? new Date(y, m - 1, d, 23, 59, 59, 999) : new Date(y, m - 1, d, 0, 0, 0, 0);
    }
    // YYYY-MM-DD HH:mm(:ss)? → ersätt ' ' med 'T' så tolkas lokalt
    if (/^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(:\d{2})?$/.test(s)) {
      const isoish = s.replace(' ', 'T');
      const dt = new Date(isoish);
      return isNaN(dt.getTime()) ? null : dt;
    }
    // ISO med Z/offset → låt JS hantera
    const dt = new Date(s);
    return isNaN(dt.getTime()) ? null : dt;
  }
...
export const DisplayWindowScreen: React.FC<DisplayWindowScreenProps> = ({ onBack, isEmbedded = false }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return posts.filter(p => {
      const start = parseMaybeDate((p as any).startDate, false);
      if (!start || start > now) return false;
      const end = parseMaybeDate((p as any).endDate, true);
      ...
      {branding?.isEnabled && selectedOrganization && (branding.showLogo || branding.showName) && (
        <div className={`absolute ${getPositionClasses(branding.position)} z-30`}>
            <div className="flex items-center gap-2 bg-black/50 backdrop-blur-sm p-1.5 rounded-md">
                ...
                {branding.showName && <p className="font-semibold text-sm text-white/90">{selectedOrganization.name}</p>}
            </div>
        </div>
      )}
