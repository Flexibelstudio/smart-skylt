...
const DisplayScreenPreviewModal: React.FC<{
    ...
    const liveScreen = useMemo(() => {
        return displayScreens.find(s => s.id === screen.id) || screen;
    ...
    const isPortrait = liveScreen.aspectRatio === '9:16' || liveScreen.aspectRatio === '3:4';
                     ...
                     */}
                     <div className={`relative shadow-2xl ring-4 ring-slate-900 rounded-lg overflow-hidden bg-black ${isPortrait ? 'h-full w-auto aspect-[9/16]' : 'w-full aspect-[16/9]'}`}>
                        <ScaledPreviewWrapper 
                            aspectRatio={liveScreen.aspectRatio}
                            className="w-full h-full"
                        >
                            <SplitScreenLayout screen={liveScreen} organization={organization}>
                                {currentPost ? (
                                    <DisplayPostRenderer 
                                        post={currentPost} 
...
const TabButton: React.FC<{
    ...
    return (
        <button
            onClick={() => setActiveTab(tabId)}
            className={`relative px-4 py-2 text-sm font-bold transition-all duration-200 focus:outline-none rounded-xl whitespace-nowrap ${isActive
                ? 'bg-teal-600 dark:bg-teal-600 text-white shadow-md shadow-teal-500/15'
...
export const SuperAdminScreen: React.FC<SuperAdminScreenProps> = (props) => {
    ...
    return (
        <div className="w-full max-w-7xl mx-auto space-y-8 animate-fade-in pb-12">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-150/70 dark:bg-slate-900/40 p-2 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-sm">
                <div className="flex overflow-x-auto scrollbar-hide min-w-0 w-full sm:w-auto p-1 gap-1.5" role="tablist">
                ...
                <div className="flex-shrink-0 self-stretch sm:self-auto flex">
                    <button 
                        onClick={() => setIsAiGuideModalOpen(true)}
                        className="w-full sm:w-auto text-xs font-bold py-2.5 px-4 bg-white dark:bg-slate-800 text-teal-600 dark:text-teal-400 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl shadow-sm hover:shadow transition-all flex items-center justify-center gap-1.5"
                    >
                        💡 AI-guide
                    </button>
                </div>
