...
function safeParseJSON<T>(text: string, schema: ZodSchema<T>): T {
  try {
    ...
    // Explicitly cast the result of schema.parse to T to satisfy TypeScript
...
async function handleAIError<T>(fn: () => Promise<T>): Promise<T> {
  try {
    ...
    // Hantera Rate Limiting (429 Resource Exhausted)
...
export async function initializeMarketingCoachChat(
  ...
  const chat = ai.chats.create({
    model: "gemini-1.5-pro", // Switched to 1.5 Pro for stability
    config: {
      systemInstruction,
...
export const generatePageContentFromPrompt = (userPrompt: string): Promise<string> =>
  handleAIError(async () => {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-1.5-pro", // Switched to 1.5 Pro
      contents: prompt,
    });
    return response.text ?? "";
  });
...
export const generateSkyltIdeas = (
  ...
  handleAIError(async () => {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-2.5-flash", // Keep Flash for speed in interactive UI
      contents: fullPrompt,
      config: {
        responseMimeType: "application/json",
  ...
  handleAIError(async () => {
    ...
    if (postData.layout !== "text-only" && (postData as any).imagePrompt) {
      const imageResponse = await ai.models.generateImages({
        model: "imagen-4.0-generate-001", // Keep Imagen 4
...
export const generateHeadlineSuggestions = (
  ...
  handleAIError(async () => {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-2.5-flash", // Keep Flash
      contents: prompt,
      config: {
        responseMimeType: "application/json",
        responseSchema: {
