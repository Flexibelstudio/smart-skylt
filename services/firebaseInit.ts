...
const forceOffline = urlParams.get('offline') === 'true' || localStorage.getItem('forceOffline') === 'true';
...
if (forceOffline || !cfg || !cfg.firebaseConfig || !cfg.firebaseConfig.apiKey) {
    resolvedEnv = 'offline';
    // If we are in a dev environment without config, we likely want Mock Data.
    if (!w.DEBUG_MODE) console.warn("App starting in OFFLINE mode. Using Mock Data.");
} else {
    try {
