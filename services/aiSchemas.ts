...
export const TagStampSuggestionSchema = z.object({
  text: RobustString,
  displayType: z.enum(['tag', 'stamp']).default('tag'),
  backgroundColor: RobustString,
  textColor: z.enum(['#FFFFFF', '#000000']).default('#FFFFFF'),
  fontSize: z.enum(['sm', 'md', 'lg', 'xl', '2xl', '3xl', '4xl', '5xl']).default('md'),
  fontFamily: z.enum(['sans', 'display', 'script', 'adscript', 'roboto', 'open-sans', 'lato', 'montserrat', 'source-sans-pro', 'nunito', 'raleway', 'oswald', 'ubuntu', 'helvetica-neue', 'arial', 'manrope', 'fira-sans', 'merriweather', 'playfair-display', 'lora', 'georgia', 'times-new-roman', 'libre-baskerville', 'eb-garamond', 'cormorant-garamond', 'pt-serif', 'dm-sans', 'work-sans', 'quicksand', 'josefin-sans', 'exo-2', 'cabin']).default('sans'),
  fontWeight: z.enum(['bold', 'black']).default('black'),
  animation: z.enum(['none', 'pulse', 'glow']).default('none'),
  shape: z.enum(['rectangle', 'circle', 'square']).optional().default('rectangle'),
  border: z.enum(['none', 'solid', 'dashed']).optional().default('none'),
  opacity: z.number().optional().default(1),
  explanation: RobustString
});

export const GenAiTagStampSuggestionSchema = {
  type: Type.OBJECT,
  properties: {
    text: { type: Type.STRING },
    displayType: { type: Type.STRING, enum: ['tag', 'stamp'] },
    backgroundColor: { type: Type.STRING },
    textColor: { type: Type.STRING, enum: ['#FFFFFF', '#000000'] },
    fontSize: { type: Type.STRING, enum: ['sm', 'md', 'lg', 'xl', '2xl', '3xl', '4xl', '5xl'] },
    fontFamily: { type: Type.STRING, enum: ['sans', 'display', 'script', 'adscript', 'roboto', 'open-sans', 'lato', 'montserrat', 'source-sans-pro', 'nunito', 'raleway', 'oswald', 'ubuntu', 'helvetica-neue', 'arial', 'manrope', 'fira-sans', 'merriweather', 'playfair-display', 'lora', 'georgia', 'times-new-roman', 'libre-baskerville', 'eb-garamond', 'cormorant-garamond', 'pt-serif', 'dm-sans', 'work-sans', 'quicksand', 'josefin-sans', 'exo-2', 'cabin'] },
    fontWeight: { type: Type.STRING, enum: ['bold', 'black'] },
    animation: { type: Type.STRING, enum: ['none', 'pulse', 'glow'] },
    shape: { type: Type.STRING, enum: ['rectangle', 'circle', 'square'] },
    border: { type: Type.STRING, enum: ['none', 'solid', 'dashed'] },
    opacity: { type: Type.NUMBER },
    explanation: { type: Type.STRING }
  },
  required: ['text', 'displayType', 'backgroundColor', 'textColor', 'fontSize', 'fontFamily', 'fontWeight', 'animation', 'explanation']
};