...
export const getCompletePostPrompt = (userPrompt: string, organization: Organization, layout?: string, isAiAd?: boolean): string => {
...
`;
  ...
  const uspsContext = (organization.preferenceProfile?.usps && organization.preferenceProfile.usps.length > 0)
    ? `\n- Unique Selling Propositions (USPs): ${organization.preferenceProfile.usps.join(', ')}`
    : "";
  ...
  const artDirectionInstruction = isAiAd
    ? `- **CRITICAL - INCLUDE TEXT IN IMAGE:** The generated image MUST contain the headline and body text beautifully integrated into the design, like a real professional advertisement. Use bold, readable typography that matches the brand. The text must be exactly the Swedish headline and body you wrote.`
...
**Brand Info:**
${brandingGuidelines}
${styleProfileContext}${targetAudienceContext}${uspsContext}
...
- 'headline' (Swedish)
- 'body' (Swedish)
- 'imagePrompt' (English, detailed, max 50 words. ${isAiAd ? 'MUST include instructions to render the exact Swedish text.' : 'NO TEXT/LETTERS in image.'})
- 'layout'
- 'backgroundColor'
- 'textColor'
...
export const getTagStampSuggestionPrompt = (
  userInput: string,
  organization: Organization,
...
- Tertiärfärg: ${organization.tertiaryColor || 'ej angiven'}
  `.trim();

  const fontsText = `
- Rubriktypsnitt: ${organization.headlineFontFamily || 'display'}
- Brödtexttypsnitt: ${organization.bodyFontFamily || 'sans'}
  `.trim();

  return `
Du är Skylie, en intelligent och hantverksskicklig varumärkesdesigner. Din uppgift är att skapa en supersnygg och professionell grafisk designtillgång för digitala skyltar i form av en ${currentType === 'tag' ? 'rektangulär Tagg' : 'rund eller kantig Stämpel'}.

**Företagsinformation & DNA-profil:**
...
Användaren vill ta fram en ${currentType === 'tag' ? 'tagg' : 'stämpel'} baserat på texten/instruktionen: "${userInput}"

**Designdirektiv för ${currentType}:**
${currentType === 'tag' 
  ? `1. En tagg är rektangulär ('shape' ska vara 'rectangle'). Den placeras vanligtvis på inlägg för att markera kategorier, statusar, etc.
2. Texten på taggen ska vara kort, kraftfull och underhållande (versaler rekommenderas starkt, t.ex. 'NYHET', 'KAMPANJ', 'KRAVMÄRKT', 'VISNING' eller '20% OFF'). Det ska INTE vara en förklarande mening, utan 1-3 korta ord maximum.
3. Välj en bakgrundsfärg (backgroundColor) som kan vara en solid hex-färg eller en CSS-färggradient (t.ex. 'linear-gradient(135deg, #primary 0%, #accent 100%)' med företagets färgteman). Den ska pop:a snyggt men ändå behålla god läsbarhet och kontrast.`
  : `1. En stämpel 'stamp' är en rund ('circle') eller kvadratisk ('square') designresurs som drar mycket blickar till sig under sändning i butik/skyltfönster.
2. Texten på stämpeln ska vara kort och rymmas snyggt (t.ex. 'SAFTIG!', 'SÅLD!', 'SÄSONG', 'VIP', 'NYHET!', 'HELGKLART'). Maximalt 1-2 korta ord, gärna med ett utropstecken eller en siffra.
3. Bakgrunden kan vara en solid färg, en mättad färggradient, eller 'rgba(255, 255, 255, 0.25)' (ljust frostat glas).
4. Stämplar ser fantastiska ut med en ram (border), antingen solid eller retro-dashed (streckad) 'dashed'.`
}

**Ytterligare krav:**
- Ge bäst förslag på lämplig text ('text'), storlek ('fontSize'), typsnitt ('fontFamily'), stämpelform ('shape'), ramstilar ('border'), opacitet ('opacity' mellan 0.8 och 1.0) samt puls/glow-animationer ('animation').
- Gör färg- och typsnittsval som känns hyper-skräddarsydda för deras bransch (t.ex. eleganta serif-typsnitt som 'merriweather' eller 'playfair-display' för lyxmäklarbyråer, eller moderna sans-serifs eller displays för runda solida butiksstämplar).
- Svara i strikt JSON-format enligt det bifogade schemat.
- Skriv en kort, trevlig förklaring på svenska ('explanation') där du berättar exakt hur du integrerade deras DNA-profil (t.ex. mäklarbranschen, deras färgskala, deras unika dragningskraft) för att skapa just denna tagg/stämpel.
`;
};