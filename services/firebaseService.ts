...
let MOCK_SUGGESTED_POSTS: SuggestedPost[] = [
    {
        id: 'sugg_1',
        automationId: 'auto_1',
        targetScreenId: 'screen_flexibel_1',
        createdAt: new Date().toISOString(),
        status: 'pending',
        postData: {
            id: 'mock_post_sugg_1',
            internalTitle: '📈 Måndagspepp: Sätt upp veckans hälsomål!',
            layout: 'text-only',
            headline: 'Måndagsutmaningen: Sätt ett mål!',
            body: 'Denna vecka utmanar vi dig att dricka ett extra glas vatten om dagen och rulla axlarna 10 gånger varje timme framför datorn. Litet steg för hälsan, stort steg för välbefinnandet!',
            durationSeconds: 15,
        }
    },
    {
        ...
        postData: {
            id: 'mock_post_sugg_2',
            internalTitle: '🧘 Kvällstips: 5 minuters andning',
            layout: 'text-only',
            headline: 'Gör ett andningsstopp ikväll',
            body: 'Innan du somnar ikväll: Sitt tyst i 5 minuter. Andas in på 4 sekunder, håll andan på 4, andas ut på 4, håll i 4. Detta sänker pulsen och lugnar nervsystemet för en djupare sömn.',
            durationSeconds: 12,
        }
    }
];

const triggerMockSuggestedPostsListener = (orgId: string) => {
    if (!isOffline) return;
    const callback = (window as any).mockSuggestedPostsListeners?.[orgId];
    if (callback) {
        callback(MOCK_SUGGESTED_POSTS);
    }
};
...
export const listenToSuggestedPosts = (orgId: string, callback: (posts: SuggestedPost[]) => void) => {
    if (isOffline || !db) {
        ...
        if (!(window as any).mockSuggestedPostsListeners) (window as any).mockSuggestedPostsListeners = {};
        (window as any).mockSuggestedPostsListeners[orgId] = callback;

        return () => {
            if ((window as any).mockSuggestedPostsListeners) {
                delete (window as any).mockSuggestedPostsListeners[orgId];
            }
        };
    }
    return db.collection('organizations').doc(orgId).collection('suggestedPosts').onSnapshot(
        ...
        error => {
            console.error("Error listening to suggested posts:", error);
            callback([]);
        }
    );
};
...
export const getSuggestedPostById = async (orgId: string, suggestionId: string): Promise<SuggestedPost | null> => {
    if (isOffline || !db) {
        return MOCK_SUGGESTED_POSTS.find(p => p.id === suggestionId) || null;
    }
...
export const updateSuggestedPost = async (orgId: string, suggestionId: string, data: Partial<SuggestedPost>) => {
    if (isOffline) {
        MOCK_SUGGESTED_POSTS = MOCK_SUGGESTED_POSTS.map(p =>
            p.id === suggestionId ? { ...p, ...data } : p
        );
        triggerMockSuggestedPostsListener(orgId);
