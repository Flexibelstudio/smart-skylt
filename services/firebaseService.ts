...
export const getSystemAnnouncements = async (): Promise<AppNotification[]> => {
  ...
  return snap.docs.map((doc) => {
    const data = doc.data();
    // FIX: Ensure the 'type' property conforms to the AppNotification type by performing a runtime check.
    const typeValue = data.type;
    const allowedTypes: AppNotification['type'][] = ['info', 'warning', 'success', 'suggestion'];
    const type: AppNotification['type'] = allowedTypes.includes(typeValue) ? typeValue : 'info';
    return {
      id: `sys-${doc.id}`,
      title: data.title,
