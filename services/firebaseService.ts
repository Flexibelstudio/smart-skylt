...
export const listenToOrganizationChanges = (
  ...
  return ref.onSnapshot(
    (s) => s.exists && onUpdate(s.data() as Organization),
...
export const updateOrganizationLogos = async (organizationId: string, logos: { light?: string; dark?: string }) => {
  if (isOffline || !db) {
...
export const updateOrganizationPostTemplates = async (organizationId: string, postTemplates: PostTemplate[]) => {
  if (isOffline || !db) {
...
export const inviteUser = async (
  ...
  try {
    ...
    const res = await fn({ organizationId, email, role: 'admin' });
    return res.data as any;
  } catch (e: any) {
    const message = e?.message || 'Ett okänt serverfel inträffade.';
    if (e?.code === 'unauthenticated') return { success: false, message: 'Du måste vara inloggad för att kunna bjuder in.' };
...
export const unpairPhysicalScreen = async (organizationId: string, physicalScreenId: string): Promise<void> => {
  if (isOffline || !db) {
    ...
    if (codeIdx > -1) {
      ...
      delete (MOCK_PAIRING_CODES[codeIdx] as any).organizationId;
      delete (MOCK_PAIRING_CODES[codeIdx] as any).assignedDisplayScreenId;
      delete (MOCK_PAIRING_CODES[codeIdx] as any).pairedByUid;
      delete (MOCK_PAIRING_CODES[codeIdx] as any).pairedAt;
    }
  ...
  await db.runTransaction(async (tr) => {
    ...
    const updated = (org.physicalScreens || []).filter((s) => s.id !== physicalScreenId);
    tr.update(orgRef, { physicalScreens: updated });
  ...
  // Viktigt: ta bort session-dokumentet så skärmen reagerar direkt
